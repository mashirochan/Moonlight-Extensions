...
const Native = moonlight.getNatives("embedXivModArchive");
...
Messages.addAccessory("embedXivModArchive", (props) => {
    const { content } = props.message;
    if (!content) return null;

    const match = content.match(XMA_REGEX);
...
function XmaEmbed({ modId, message, channel }: { modId: string; message: Message; channel?: any }) {
    ...
    const [thumbSrc, setThumbSrc] = React.useState<string | undefined>(undefined);
                ...
                <div style={{ padding: 8, minWidth: 384 }}>
                    ...
                    {data &&
                        (() => {
                            ...
                            const downloadHref = link
                                ? link.startsWith("http")
                                    ? link
                                    : `https://www.xivmodarchive.com${link}`
                                : undefined;
                            return (
                                <div
                                    style={{
                                ...
                                >
                                    {(thumbSrc || data.thumbnail) && (
                                        <div style={{ position: "relative", display: "inline-block" }}>
                                            <img
                                                ...
                                                onClick={() =>
                                                    window.open(
                                                        `https://www.xivmodarchive.com/modid/${modId}`,
                                                        "_blank"
                                                    )
                                                }
                                            />
                                    ...
                                    <div style={{ flex: 1, display: "flex", width: "100%" }}>
                                        <div style={{ display: "flex", flexDirection: "column", width: "33%" }}>
                                            ...
                                            <div style={{ fontSize: 12 }}>
                                                {parseInt(data?.views)?.toLocaleString() ?? "Unknown"}
                                            </div>
                                        </div>
                                        <div style={{ display: "flex", flexDirection: "column", width: "33%" }}>
                                            <div style={{ fontSize: 12, fontWeight: 600 }}>Downloads</div>
                                            <div style={{ fontSize: 12 }}>
                                                {data?.downloads?.toLocaleString() ?? "Unknown"}
                                            </div>
                                        </div>
                                        <div style={{ display: "flex", flexDirection: "column", width: "33%" }}>
                                            <div style={{ fontSize: 12, fontWeight: 600 }}>Followers</div>
                                            <div style={{ fontSize: 12 }}>
                                                {data?.followers?.toLocaleString() ?? "Unknown"}
                                            </div>
                                        </div>
                                    </div>
                                    <div style={{ flex: 1, display: "flex", width: "100%" }}>
                                        <div style={{ display: "flex", flexDirection: "column", width: "33%" }}>
                                            ...
                                            <div style={{ fontSize: 12 }}>
                                                {new Date(parseInt(data?.time_published))?.toLocaleDateString() ??
                                                    "Unknown"}
                                            </div>
                                        </div>
                                        <div style={{ display: "flex", flexDirection: "column", width: "33%" }}>
                                            ...
                                            <div style={{ fontSize: 12 }}>
                                                {new Date(parseInt(data?.time_version_updated))?.toLocaleDateString() ??
                                                    "Unknown"}
                                            </div>
                                        </div>
                                        <div style={{ display: "flex", flexDirection: "column", width: "33%" }}>
                                    ...
                                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                                        <LinkButton size="small" href={`https://www.xivmodarchive.com/modid/${modId}`}>
                                            View on XMA
                                        </LinkButton>
                                        <LinkButton size="small" href={downloadHref}>
                                            Download
                                        </LinkButton>
                                        {(data.nsfw || data.nsfl) && (
                                            <div
